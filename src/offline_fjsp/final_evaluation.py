...
def run_final_campaign(
    *,
    ...
    for model_seed in model_seeds:
        cql = NeuralCQL(hidden_dim=32, seed=model_seed).fit(
            dataset,
            epochs=neural_epochs,
        )
        iql = NeuralIQL(hidden_dim=32, seed=model_seed).fit(
            dataset,
            epochs=neural_epochs,
        )
        learned.append((model_seed, "neural_cql", cql.act))
        learned.append((model_seed, "neural_iql", iql.act))

    rows: list[FinalRow] = []
    blocks = (
        ("nominal", nominal_final),
        ("ood_scale", ood_scale),
        ("ood_flexibility", ood_flexibility),
    )
    for scenario, seeds in blocks:
        for seed in seeds:
            rows.append(
                _evaluate_policy(seed, scenario, -1, "minimum_slack", minimum_slack)
            )
        ...
        output.append(
            {
                ...
                "mean_weighted_tardiness": float(
                    np.mean([r.weighted_tardiness for r in selected])
                ),
...
def paired_against_minimum_slack(rows: list[FinalRow]) -> list[dict[str, float | str]]:
    ...
    for scenario in sorted({row.scenario for row in rows}):
        reference_rows = [
            r for r in rows if r.scenario == scenario and r.policy == "minimum_slack"
        ]
        reference = {r.instance_seed: r.weighted_tardiness for r in reference_rows}
        policies = {r.policy for r in rows if r.scenario == scenario} - {"minimum_slack"}
        for policy in sorted(policies):
            ...
            for seed in reference:
                values = [
                    r.weighted_tardiness
                    for r in candidate_rows
                    if r.instance_seed == seed
                ]
                if values:
